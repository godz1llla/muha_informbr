...
class Router
{
    ...
    public function dispatch()
    {
        ...
        if ($segments[0] === 'post' && isset($segments[1]) && $segments[1] === 'submitComment') {
            return ['controller' => 'PostController', 'method' => 'submitComment', 'params' => []];
        }

        // Поиск: /search или /ru/search
        if ($segments[0] === 'search') {
            return ['controller' => 'SearchController', 'method' => 'index', 'params' => []];
        }

        // Архив видео: /videos или /ru/videos
        if ($segments[0] === 'videos') {
            return ['controller' => 'VideoController', 'method' => 'index', 'params' => []];
        }

        // Жарнама (Реклама): /ads или /ru/ads
        if ($segments[0] === 'ads') {
            return ['controller' => 'AdController', 'method' => 'index', 'params' => []];
        }

        // Редакция: /redaktsiya или /ru/redaktsiya
        if ($segments[0] === 'redaktsiya') {
            return ['controller' => 'HomeController', 'method' => 'editor', 'params' => []];
        }
    ...
    private function handleAdminRoutes($segments)
    {
        ...
        switch ($action) {
            ...
            case 'media':
                $method = isset($segments[1]) ? $segments[1] : 'index';
                $params = array_slice($segments, 2);
                return ['controller' => 'admin\AdminMediaController', 'method' => $method, 'params' => $params];
            case 'users':
                $method = isset($segments[1]) ? $segments[1] : 'index';
                $params = array_slice($segments, 2);
                return ['controller' => 'admin\AdminUserController', 'method' => $method, 'params' => $params];
            case 'settings':
                $method = isset($segments[1]) ? $segments[1] : 'index';
                $params = array_slice($segments, 2);
                return ['controller' => 'admin\AdminSettingsController', 'method' => $method, 'params' => $params];
            case 'videos':
                $method = isset($segments[1]) ? $segments[1] : 'index';
                $params = array_slice($segments, 2);
                return ['controller' => 'admin\AdminVideoController', 'method' => $method, 'params' => $params];
            case 'ads':
                $method = isset($segments[1]) ? $segments[1] : 'index';
                $params = array_slice($segments, 2);
                return ['controller' => 'admin\AdminAdsController', 'method' => $method, 'params' => $params];
            default:
                return ['controller' => 'admin\AdminDashboardController', 'method' => 'index', 'params' => []];
        }
    }
